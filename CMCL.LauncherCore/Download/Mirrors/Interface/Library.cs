...
namespace CMCL.Core.Download.Mirrors.Interface
{
    public abstract class Library
    {
        ...
        /// <returns></returns>
        public virtual async ValueTask DownloadLibrariesAsync(
            List<(string savePath, string downloadUrl)> librariesToDownload)
        {
            ...
            string versionId, bool checkBeforeDownload = false, bool containsNative = true)
        {
            ...
            var taskArray = libraries.Select(l => Task.Run(async () =>
            {
                try
                {
                    ...
                    if (!l.IsNative)
                    {
                        var savePath = Utils.CombineAndCheckDirectory(true, basePath, l.Downloads.Artifact.Path);
                        //转换地址
                        var url = TransUrl(l.Downloads.Artifact.Url);
                        if (checkBeforeDownload && File.Exists(savePath) && string.Equals(
                            await Utils.GetSha1HashFromFileAsync(savePath).ConfigureAwait(false),
                    ...
                    else if (containsNative)
                    {
                        ...
                        var savePath = Utils.CombineAndCheckDirectory(true, basePath, nativeInfo.Path);
                        //转换地址
